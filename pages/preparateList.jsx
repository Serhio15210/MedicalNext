...
const PreparateList = () => {
    ...
    const fetchData = async (page = router.query.page || 1, perPage = router.query.perPage || 20) => {
        router.push(`/preparateList?page=${page}&perPage=${perPage}`, undefined, { shallow: true })
        let query = []
    ...
    return (
        <Layer>
            <div className='container'>
                ...
                <Table columns={columns} data={data} handlePageChange={fetchData} setPerPage={setPerPage} router={router}
                path="/preparateList/preparateUserList"/>
            </div>
        </Layer>
    )
}

export default PreparateList
